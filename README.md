...
- **알림·행동요령**: 공식 알림은 KST 기준 당일·전일 재난문자와 기상특보만 최신순으로 합쳐 페이지당 10건을 표시합니다. 별도 DB에는 저장하지 않으며 서버 응답과 브라우저 임시 캐시에서도 보관 범위를 벗어난 항목을 제외합니다. 행동요령 상단은 재난문자·기상특보·날씨를 조합해 하루 1회 가장 가까운 유형을 추천합니다.
...
| `BIZROUTER_API_KEY` | BizRouter 서버 비밀 API 키(2차 판정, 챗봇, 임베딩, STT) |
| `BIZROUTER_BASE_URL` | 선택값. BizRouter OpenAI 호환 기본 주소 `https://api.bizrouter.ai/v1` |
| `BIZROUTER_CHAT_MODEL` / `BIZROUTER_EMBEDDING_MODEL` | BizRouter의 `provider/model` 형식 모델 ID. `.env.example` 기본값 참고 |
...
- **기상특보(주의보/경보)**: `wrn_now_data.php`를 API 허브 문서의 컬럼 순서(REG_UP, REG_UP_KO, REG_ID, REG_KO, TM_FC, TM_EF, WRN, LVL, CMD)로 파싱합니다. typ01 응답은 UTF-8이 아닌 EUC-KR(CP949)일 수 있으므로 원시 바이트를 응답 charset에 맞춰 디코딩한 뒤 WRN/LVL 코드를 한글로 변환합니다 (`src/lib/kma.ts`의 `getWeatherAlerts`).
...
- **카카오 SDK 응답 판별**: `window.kakao=window.kakao||{}`로 시작하는 압축 JavaScript는 오류가 아니라 정상 부트스트랩입니다. 앱은 `autoload=false`로 한 번만 삽입하고 `kakao.maps.load()` 콜백 안에서 지도를 생성합니다. 이후 `tilesloaded`까지 확인하므로 SDK·지도 본체·타일 CDN 중 어느 단계가 실패했는지 긴 스크립트 원문 대신 짧은 진단 코드로 표시합니다.
...
- **재난문자(safetydata.go.kr)**: 긴급재난문자 DSSP-IF-00247은 `SN/CRT_DT/MSG_CN/RCPTN_RGN_NM`, 재난문자(속보) DSSP-IF-10748은 `MSTN_BRNE_NO/REG_DT/MSTN_BRNE_CN`을 각각 정규화해 병합합니다. KST 전일부터 필요한 페이지를 끝까지 조회하고 당일·전일만 반환합니다. 10748에 별도 지역 배열이 없으면 본문 끝의 `[발신지역]`과 본문의 공식 지역명을 추출합니다 (`src/lib/safetydata.ts`).
...
- **BizRouter**: OpenAI 호환 Chat Completions 요청에는 빈 문자열이 아닌 system/user 메시지를 모두 전달합니다. 응답 텍스트가 비어 있거나 호출이 실패하면 성공처럼 보이는 `{ text: "", fallback: true }`만 반환하지 않고 관리자 화면에 모델·인증·응답 형식을 확인할 수 있는 비밀값 없는 원인을 함께 표시합니다.
...
- [ ] 재난안전데이터공유플랫폼에서 DSSP-IF-00247·10748 이용신청이 각각 승인되었고 전용 키 두 개가 Vercel에 등록됐는지 확인
...
- [ ] 관리자 API 테스트에서 긴급재난문자·속보의 최신 시각, 비어 있지 않은 내용·지역과 기상특보 한글을 확인
- [ ] 관리자 API 테스트에서 BizRouter 응답의 `fallback=false`와 비어 있지 않은 `text`를 확인
