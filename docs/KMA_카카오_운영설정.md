...
특보현황 `wrn_now_data.php`는 typ01 텍스트 자료이며 응답에 따라 EUC-KR(CP949) 바이트를 반환할 수 있습니다. `Response.text()`는 UTF-8로 해석하므로 그대로 사용하면 지역명과 오류 안내의 한글이 깨집니다. 앱은 `arrayBuffer()`로 원시 바이트를 받은 뒤 `Content-Type`의 charset을 우선하고, charset이 없으면 유효한 UTF-8인지 확인한 후 EUC-KR로 폴백합니다.

관리자 테스트의 기상특보 결과에서 `REG_UP_KO/REG_KO`에 해당하는 지역명이 정상 한글인지, `�` 또는 `媛뺥뭾` 같은 깨진 문자열이 없는지 확인합니다.
...
- [ ] `ADMIN-KMA-007` `/admin` 기상특보 테스트에서 특보 종류·단계·지역명이 깨지지 않은 한글로 표시되는지 확인
...
React 화면에서는 SDK 주소에 `autoload=false`를 지정해 부트스트랩의 `document.write()` 자동 로드를 막고, 스크립트의 `load` 완료 뒤 `kakao.maps.load(callback)` 안에서만 `LatLng`, `Map`, `Marker`를 만듭니다. 현재 지도 렌더링 자체에는 추가 라이브러리가 필요하지 않으므로 `libraries=services`를 넣지 않습니다. 병원·약국과 좌표 변환은 브라우저 services 라이브러리가 아니라 서버의 카카오 Local REST API를 사용합니다.
...
| 지도 타일 로딩 시간 초과 | `mts.daumcdn.net` 타일 PNG 요청, 추적 방지·광고 차단기·방화벽·CSP의 이미지 허용 여부 |
...
- [ ] `ADMIN-KAKAO-012` 개발자 도구에서 `sdk.js` → 지도 본체 `kakao.js` → `mts.daumcdn.net` 타일 PNG가 모두 성공하고 화면 진단 코드가 남지 않는지 확인
